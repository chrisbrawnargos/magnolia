...
[#macro relatedTours categoryName tours]
    ...
    <div class="container after-category-header">
        ...
        <h2>${i18n.get('tour.featured', [categoryName])}</h2>
            ...
            [#if cmsfn.hasTemplateOfType(page, "home")]
                <p class="text-center">${i18n.get('tour.featured.preference', [categoryName])} <a href="#" onclick="removeTourTypePreference()">${i18n.get('tour.featured.clearPreference')}</a></p>
            [/#if]
